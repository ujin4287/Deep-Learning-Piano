...
PATH = './save_model/piano_model_210523.pth'
...
def video_capture():
    ...
    while True:
        ...
        if time.time() - start_time >= capture_time:
            img_name = "temp.jpg"
            cv2.imwrite(img_name, frame) # OpenCV 이므로 BGR 순으로 저장된다.
            # print("captured")
            start_time = time.time()
...
if __name__ == '__main__':
    ...
    t1 = Thread(target=video_capture, args=())
    t1.start()
    ...
    while True:
        ...
        original_image = original_image.convert('RGB') # RGB 로 변환
